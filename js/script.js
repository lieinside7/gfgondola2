...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Proteksi terhadap clickjacking
  if (self === top) {
...
// Tambahkan CSP nonce untuk script inline
(function() {
  const nonce = crypto.getRandomValues(new Uint32Array(1))[0].toString(16);
  document.querySelector('script[nonce]')?.setAttribute('nonce', nonce);
  const meta = document.createElement('meta');
  meta.httpEquiv = 'Content-Security-Policy';
  meta.content = `script-src 'nonce-${nonce}' 'strict-dynamic' https: http: 'unsafe-inline'`;
